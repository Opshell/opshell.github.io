...
function normalizePromo(raw: Raw): PromoCode {
    return {
        ...
        plan_tier: field(raw, 'plan_tier', 'PlanTier', ''),
...
export const adminApi = {
    ...
    updateDevice: async (token: string, id: number, patch: DevicePatch) =>
    ...
    // #region [P] 審回報
    feedbackStats: async (token: string) => request<FeedbackStats>(token, '/v1/admin/feedback/stats'),
    ...
    getFeedback: async (token: string, id: number) => request<{ report: FeedbackReport }>(token, `/v1/admin/feedback/${id}`),
    ...
    reviewFeedback: async (token: string, id: number, patch: { status?: FeedbackStatus; issue_id?: number | null }) =>
     ...
     */
    batchReviewFeedback: async (token: string, body: ({ report_ids: number[] } | { device_id: number }) & { status: FeedbackStatus }) =>
    ...
    updateIssue: async (token: string, id: number, body: { title?: string; weight?: number; note?: string }) =>
