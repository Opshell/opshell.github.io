<script setup lang="ts">
    ...
    const themeColor = computed(() => props.color || '#00f0ff');
        ...
        const points = [
            [fx(5), 0], [fx(_w - 5), 0], [fx(_w), 5],             // 頂部
            [fx(_w), midY - 30], [fx(_w - SLOPE), midY - 20],        // 內折開始 (dx=10, dy=10)
            [fx(_w - SLOPE), _h - SLOPE], [fx(_w - 2 * SLOPE), _h],    // 右下切角 (dx=10, dy=10)
            [fx(15), _h], [fx(0), _h - 15],                          // 左下
            [fx(0), 5]                                           // 左上
        ];
    ...
    <div ref="container" class="industrial-hud" :style="{ '--hud-color': themeColor }">
        ...
        <div class="hud-content">
            <header class="hud-header">
                ...
                <span class="title-text">{{ title }} header text</span>
            </header>
            <main class="hud-main">
                <slot />
            </main>
