---
title: '串 API 的那些事（二）：取名是小事，也是大事'
image: ''
description: 'useApi.ts 這個名字有錯嗎？沒有，但只串一個後端時才夠用。當專案開始接金流、地圖、公開資料，命名要回答「這是哪個 API」：比較 useBackendApi、useHttpClient 與 useFetch。'
keywords: ''
author: Opshell
...
**這篇的脈絡**：上一篇重構完，群組裡接著有人問：「那這個檔案叫 useApi.ts 好嗎？」命名不會讓程式跑不動，但會決定下一個接手的人要花多久才看懂。
:::

檔案命名、變數命名，這種看似「小事」的地方，其實最能體現一個**前端工程師**{.vue}的經驗和對專案架構的思考深度。
...
在一個小到中型的專案裡，如果整個系統只有一個主要的後端 API 來源，那 useApi.ts 其實非常直觀，大家都能理解它就是用來跟我們主要的**後端**{.info}溝通的工具。
...
因為它最貼近日常溝通的語言，**前端工程師**{.vue}和**後端**{.info}工程師在討論問題時，會說「你打一下後端的 User API」，而不會說「你用一下 HTTP Client 去 call User Endpoint」。useBackendApi.ts 能讓程式碼和我們的溝通語言保持一致，這在團隊協作中非常有價值。
