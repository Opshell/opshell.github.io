---
title: '告別俄羅斯娃娃：用工廠模式與 Slot 打造優雅的 Vue 動態表單'
image: ''
description: '表單欄位依權限動態增減，不要在 template 裡塞滿 v-if：用工廠函式加 computed 把 UI 結構配置化；元件可以拆小但要拉扁，用 slot 打斷深層嵌套與 prop drilling。'
keywords: ''
author: Opshell
...
---
::: info 這篇的脈絡
這篇來自群組裡一次關於「動態表單欄位」的討論：有人的表單要依 VIP 身分顯示不同欄位，template 已經被 v-if 淹沒。大家各自講了自己的做法，我把共識整理成三件事：UI 配置化、元件拉扁、善用 slot。原話收在文末附錄。
:::
...
::: details 附錄：當初群組討論的原話
這篇是從這段討論整理出來的，原話留著，有些細節（表單驗證、options 放 store、children route）文章裡沒展開。
